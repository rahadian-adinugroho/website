...
async function findExistingComment(prNumber: string): Promise<number | null> {
  const res = await fetch(
    `${GITHUB_API}/repos/${process.env.GITHUB_REPOSITORY}/issues/${prNumber}/comments`,
    {
      headers: {
...
async function updateComment(
  ...
  const res = await fetch(
    `${GITHUB_API}/repos/${process.env.GITHUB_REPOSITORY}/issues/comments/${commentId}`,
    {
      method: "PATCH",
      headers: {
        Authorization: `Bearer ${process.env.GITHUB_TOKEN}`,
...
async function main() {
  ...
  const relevant = changedFiles
    ...
    .filter((x): x is { path: string; coverage: FileCoverage } => x !== null);
